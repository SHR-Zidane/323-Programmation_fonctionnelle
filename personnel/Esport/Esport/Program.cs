...
static void Main(string[] args)
{
    ...
    var valorant = DataSeries.DataSeries<ValorantMatch>.From(valorantMatches);
    Console.WriteLine(valorant.Count); // 3



    var cs2 = DataSeries.DataSeries<Cs2Match>.From(new[]
    {
    ...
    var lol = DataSeries.DataSeries<LolMatch>.From(new[]
    {
        new LolMatch("Noé", "Thresh", 2, 4, 18, 42, 71, true),
        new LolMatch("Noé", "Thresh", 1, 6, 12, 35, 64, false),
    });

    Console.WriteLine($"CS2 : {cs2.Count} matchs, LoL : {lol.Count} matchs"); 
